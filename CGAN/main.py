...
if args.arch == "my":
    from Nets import *
    netG = Generator(num_classes, (nc, ndf, ndf)).to(device)
    netG.apply(weight_init)
    netD = Discriminator(num_classes, (nc, ndf, ndf)).to(device)
...
for epoch in tqdm(range(start_epoch, args.epochs+1)):
    train(args, device, train_loader, epoch, netD, netG,nz , ndf, nc, optimizerD, optimizerG, batches_done, num_classes) 
