...
def fgsm_method(image, epsilon, data_grad):  # defining the method
    ...
    )  # peturbation according to adding noise multiplied with a small number
...
epsilons = [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3]  # defining epsilons
...
def test(model, device, test_loader, epsilon):
    ...
    print(
        f"Epsilon: {epsilon}\t Test accur = {correct}/{len(test_loader)} = {final_acc}"
    )
