...
class LitModel(pl.LightningModule):
    def __init__(
        self, num_classes, upscale_factor=2, learning_rate=0.1, weight_decay=0.0001
    ):
        super().__init__()
...
class ImageClassDs(Dataset):
    def __init__(self, img_dir, mask_dir, train: bool = True, transforms=None):
    ...
    def __getitem__(self, index):
        ...
        im = Image.open(f"{self.img_dir}/{im_path}").convert("YCbCr")
        #         im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)

        #         ma = cv2.imread(f"{self.mask_dir}/{mask_path}", cv2.IMREAD_COLOR)
        ma = Image.open(f"{self.mask_dir}/{mask_path}").convert("YCbCr")
        ...
        return torch.from_numpy(im).float(), torch.from_numpy(ma).float()
